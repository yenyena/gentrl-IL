...
class LP(nn.Module):
    ...
    def __make_contr_vec(x, var, missed, means, log_stds):
        if missed is None:
            ...
            missed = torch.isnan(x).to(torch.bool)
            x[missed] = 0
        if var[0] == 'd':
            contr_vect = torch.zeros(x.shape[0], var[1], device="cuda:0")
            contr_vect[missed] = 1
            contr_vect[torch.arange(x.shape[0]), x.long().cpu()] = 1
        elif var[0] == 'c':
            cur_vals = x[:, None].to("cuda:0")
            cur_stds = torch.exp(log_stds)[None, :].to("cuda:0")
            cur_means = means[None, :].to("cuda:0")
            ...
            contr_vect = contr_vect.to("cuda:0")

            m = missed.float()[:, None].to("cuda:0")
        ...
        for i, (core, var) in enumerate(zip(perm_cores, perm_dist_descr)):
            ...
            if perm_marg[i]:
                ...
                cur_contr_vect = self.__make_contr_vec(perm_x[:, i],
                                                       var, None,
                                                       perm_means[i],
                                                       perm_log_stds[i]).to("cuda:0")
                cond_core = core[None, :, :, :].to("cuda:0")
                cond_core = cond_core * cur_contr_vect[:, :, None, None]
                cond_core = cond_core.sum(dim=1)

            norm_core = core.sum(dim=0).to("cuda:0")
            cond_core = cond_core.to("cuda:0")
            pref = pref.to("cuda:0")

            pref = torch.bmm(pref, cond_core)
            norm_pref = norm_pref @ norm_core

            cur_norm_const = (torch.sum(norm_pref) + self.eps).to("cuda:0")
    ...
    def sample(self, num_samples, sample_descr, conds=None):
        ...
        for i, (action, var, core, suf, prev_contr_vect) in enumerate(
                zip(perm_sample_descr,
                    ...
                    sufxs,
                    contr_vect_list)):
            core = self._pos_func(core).to("cuda:0")
            if action == 's':
                # compute current mixture/discr dist weights
                part_to_contract = torch.bmm(suf.to("cuda:0"), pref).permute(0, 2, 1)
                part_to_contract = part_to_contract[:, None, :, :].to("cuda:0")
                ...
                samples_list.append(cur_samples.to("cuda:0"))
            ...
            cond_core = (core[None, :, :, :] * contr_vect[:, :, None, None])
            cond_core = cond_core.sum(dim=1).to("cuda:0")
    ...
    def reinit_from_data(self, data, var_types=None):
        ...
        for i, var_descr in enumerate(self.distr_descr):
            ...
            if var_descr[0] == 'd':
                ...
                gmm.fit(cur_data.reshape(-1, 1))
                cur_components = gmm.predict(cur_data.reshape(-1, 1))
